...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page">' +
            ...
            '</li>' + 
            '<li ng-show="conf.totalItems > 0">' + 
            '<select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()" style="width:45px;"></select>' + 
            '</li>' + 
            '<li ng-show="conf.totalItems > 0">' + 
            '<span>/共<strong>{{ conf.totalItems }}</strong>条</span>' + 
            '</li>' + 
            '</ul>',
        replace: true,
        scope: {
